...
func TestQuerier_head만_있을_때_읽는다(t *testing.T) {
	...
	m, _ := labels.NewMatcher(labels.MatchEqual, labels.MetricName, "node_load1")
	got := q.Select(m)
	if len(got) != 1 {
...
func TestQuerier_라벨_이름과_값을_모아준다(t *testing.T) {
	...
	q := NewQuerier(0, 1<<62, h, nil)
	names := q.LabelNames()
	if len(names) != 3 || names[0] != labels.MetricName || names[1] != "node" || names[2] != "tier" {
		t.Fatalf("LabelNames: got %v", names)
	}
	if vals := q.LabelValues(labels.MetricName); len(vals) != 2 || vals[0] != "a" || vals[1] != "b" {
...
func TestQuerier_head_동시_Reset중_라벨조회가_안전하다(t *testing.T) {
	...
	for {
		...
		default:
			_ = q.LabelNames()
			_ = q.LabelValues(labels.MetricName)
		}
	}
}
