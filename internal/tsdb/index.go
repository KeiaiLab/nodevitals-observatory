...
func (p *memPostings) Add(id uint64, ls labels.Labels) {
	p.mtx.Lock()
	defer p.mtx.Unlock()
	for _, l := range ls {
		vals, ok := p.m[l.Name]
...
// matchesAll 로 한다.
func selectRefs(p *memPostings, ms []*labels.Matcher) []uint64 {
...
func matchesAll(ls labels.Labels, ms []*labels.Matcher) bool {
	for _, m := range ms {
