...
func TestDB_무작위_샘플_왕복_동일성(t *testing.T) {
	...
	for k, ws := range want {
		mn, _ := labels.NewMatcher(labels.MatchEqual, "node", k.node)
		mm, _ := labels.NewMatcher(labels.MatchEqual, labels.MetricName, k.metric)
		got := q.Select(mn, mm)
		if len(got) != 1 {
...
func TestDB_Compact가_블록과_롤업을_만든다(t *testing.T) {
	...
	m, _ := labels.NewMatcher(labels.MatchEqual, "node", "e101")
	raw, _ := labels.NewMatcher(labels.MatchEqual, RollupLabel, "")
	got := q.Select(m, raw)
	if len(got) != 1 {
	...
	// 롤업 시리즈도 존재해야 한다.
	sumM, _ := labels.NewMatcher(labels.MatchEqual, RollupLabel, "sum")
...
func TestDB_재오픈시_knownRefs가_복원돼_시리즈_레코드가_중복되지_않는다(t *testing.T) {
	...
	samples := 0
	if err := ReplayWAL(walDir(dir),
		func(uint64, labels.Labels) error { seriesRecords++; return nil },
