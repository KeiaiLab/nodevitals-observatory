...
func TestSelectRefs_색인으로_후보를_좁힌다(t *testing.T) {
	...
	re, _ := labels.NewMatcher(labels.MatchRegexp, "tier", "core|gpu")
	if got := selectRefs(p, []*labels.Matcher{re}); !reflect.DeepEqual(got, []uint64{1, 2, 3}) {
		t.Fatalf("tier=~core|gpu: got %v", got)
	}

	tierEq, _ := labels.NewMatcher(labels.MatchEqual, "tier", "gpu")
	if got := selectRefs(p, []*labels.Matcher{eq, tierEq}); !reflect.DeepEqual(got, []uint64{3}) {
...
func TestSelectRefs_색인으로_좁힐_수_없으면_전체를_준다(t *testing.T) {
	...
	if got := selectRefs(p, []*labels.Matcher{ne}); !reflect.DeepEqual(got, []uint64{1, 2}) {
		t.Fatalf("tier!=gpu 후보: got %v, want [1 2]", got)
	}

	// 빈 값 같음-매처(tier="")도 마찬가지다. 색인에는 빈 값 posting 이
	// 없으므로 시드로 쓰면 결과가 통째로 사라진다.
	empty, _ := labels.NewMatcher(labels.MatchEqual, "tier", "")
	if got := selectRefs(p, []*labels.Matcher{empty}); !reflect.DeepEqual(got, []uint64{1, 2}) {
...
func TestMatchesAll(t *testing.T) {
	...
	eq, _ := labels.NewMatcher(labels.MatchEqual, "node", "e101")
	ne, _ := labels.NewMatcher(labels.MatchNotEqual, "tier", "gpu")
	if !matchesAll(ls, []*labels.Matcher{eq, ne}) {
		t.Fatal("둘 다 만족해야 한다")
	}

	bad, _ := labels.NewMatcher(labels.MatchEqual, "node", "e102")
	if matchesAll(ls, []*labels.Matcher{eq, bad}) {
		t.Fatal("하나라도 어긋나면 false 여야 한다")
	}

	// 없는 라벨은 빈 문자열로 취급된다.
	emptyEq, _ := labels.NewMatcher(labels.MatchEqual, "device", "")
	if !matchesAll(ls, []*labels.Matcher{emptyEq}) {
...
func TestSelectRefs_정규식_아무것도_안_맞기(t *testing.T) {
	...
	if got := selectRefs(p, []*labels.Matcher{re}); len(got) != 0 {
...
func TestSelectRefs_부정_정규식은_색인으로_좁힐_수_없음(t *testing.T) {
	...
	if got := selectRefs(p, []*labels.Matcher{notRe}); !reflect.DeepEqual(got, []uint64{1, 2}) {
		t.Fatalf("tier!~core: got %v, want [1 2]", got)
	}
}
