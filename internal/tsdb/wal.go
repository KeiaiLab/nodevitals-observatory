...
func (w *WAL) LogSeries(ref uint64, ls labels.Labels) error {
	buf := make([]byte, 0, 64)
...
func parseSeries(p []byte) (uint64, labels.Labels, error) {
	if len(p) < 10 {
...
// 전파한다.
func ReplayWAL(dir string, onSeries func(uint64, labels.Labels) error, onSamples func([]RefSample) error) error {
	segs, err := listSegments(dir)
	if err != nil {
		return err
	}
