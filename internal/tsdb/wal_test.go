...
func TestWAL_CRC_손상_감지_테스트(t *testing.T) {
	dir := t.TempDir()
	w, _ := OpenWAL(dir, defaultSegmentSize)
	w.LogSeries(1, NewLabels(Label{MetricName, "test"}))
	...
	nSamples := 0
	err := ReplayWAL(dir,
		func(uint64, Labels) error { nSeries++; return nil },
...
func TestWAL_특수_바이트_라벨_왕복(t *testing.T) {
	...
	specialLabels := NewLabels(
		Label{MetricName, "test\nwith\nnewlines"},
		Label{"key", "value\x00with\x00nulls"},
		Label{"binary", "\xff\xfe\xfd"},
	)
