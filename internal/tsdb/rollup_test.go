...
func TestRollupSeries_원본_라벨을_보존한다(t *testing.T) {
	...
	out := RollupSeries(h.Select(m), rollupInterval)
	s := out[0]
	if s.lset.Get(labels.MetricName) != "gpu_temp" || s.lset.Get("node") != "e22" || s.lset.Get("device") != "gpu3" {
...
func TestRollupSeries_여러_버킷_테스트(t *testing.T) {
	...
	m, _ := labels.NewMatcher(labels.MatchEqual, "node", "e1")
	out := RollupSeries(h.Select(m), rollupInterval)
	if len(out) != 4 {
...
func TestRollupSeries_count로_평균_유도(t *testing.T) {
	...
	m, _ := labels.NewMatcher(labels.MatchEqual, "test", "avg")
	out := RollupSeries(h.Select(m), rollupInterval)
