...
func TestHead_매처로_시리즈를_고른다(t *testing.T) {
	...
	re, _ := labels.NewMatcher(labels.MatchRegexp, "node", "e10.")
	tierEq, _ := labels.NewMatcher(labels.MatchEqual, "tier", "gpu")
	got = h.Select(re, tierEq)
	if len(got) != 1 || got[0].lset.Get(labels.MetricName) != "gpu_temp" {
		t.Fatalf("node=~e10. + tier=gpu: got %d 시리즈", len(got))
	}

	neq, _ := labels.NewMatcher(labels.MatchNotEqual, "tier", "core")
	got = h.Select(neq)
	if len(got) != 1 {
