...
func latestSample(t *testing.T, q *tsdb.Querier, name, instance string) float64 {
	t.Helper()
	mName, err := labels.NewMatcher(labels.MatchEqual, labels.MetricName, name)
	if err != nil {
		t.Fatalf("NewMatcher(%q): %v", name, err)
	}
	mInst, err := labels.NewMatcher(labels.MatchEqual, instanceLabel, instance)
	if err != nil {
...
func TestScrapeOnce_저장과_라벨매핑(t *testing.T) {
	...
	m, err := labels.NewMatcher(labels.MatchEqual, labels.MetricName, "node_load1")
	if err != nil {
...
func TestScrapeOnce_실패타겟_격리와_up메트릭(t *testing.T) {
	...
	mLoad, err := labels.NewMatcher(labels.MatchEqual, labels.MetricName, "node_load1")
	if err != nil {
		t.Fatalf("NewMatcher: %v", err)
	}
