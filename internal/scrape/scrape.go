...
func NewScraper(db *tsdb.DB, d discovery.Discoverer, opts Options) *Scraper {
	if opts.Interval <= 0 {
...
func buildTargetLabels(sr promtext.Series, instance string) tsdb.Labels {
	m := make(map[string]string, len(sr.Labels)+2)
	for k, v := range sr.Labels {
		m[k] = v
	}
	m[tsdb.MetricName] = sr.Name
...
func (s *Scraper) appendSelfMetric(name, instance string, t int64, v float64) {
	lset := tsdb.LabelsFromMap(map[string]string{
		tsdb.MetricName: name,
		instanceLabel:   instance,
	})
