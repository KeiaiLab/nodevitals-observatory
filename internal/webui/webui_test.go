...
func TestHandler_index참조_해시자산_전부_200(t *testing.T) {
	// 번들은 커밋하지 않는다(내부 gitignore) — `make web-build` 가 채운다.
	// 빌드 전에는 검사할 대상 자체가 없으므로 건너뛴다. 이 가드가 없으면
	// 프론트를 빌드하지 않은 개발자의 `make all` 이 항상 실패하고, 그러면
	// 사람들이 게이트 자체를 신뢰하지 않게 된다.
	if !bundlesPresent(t) {
		t.Skip("프론트 미빌드 — make web-build 후 검사한다")
	}
...
// bundlesPresent 는 임베드된 assets 에 실제 번들(js/css)이 있는지 본다.
// index.html 셸만 있으면 false 다.
func bundlesPresent(t *testing.T) bool {
	t.Helper()
	...
	for _, e := range entries {
		if e.IsDir() && e.Name() == "assets" {
			sub, err := fs.ReadDir(FS(), "assets")
			if err != nil {
				return false
			}
			for _, f := range sub {
				if strings.HasSuffix(f.Name(), ".js") || strings.HasSuffix(f.Name(), ".css") {
					return true
				}
			}
		}
