...
/** 중심 이동평균 — 계열 길이는 유지하고 끝점도 버리지 않는다(창을 잘라 평균).
 *  스파크라인·추이선의 고주파 노이즈만 제거하는 용도. */
...
export default function GpuServing() {
  ...
  const fleetTrend = usePolledQuery(
    async () => {
      ...
      // 토큰·에러율도 같은 창으로 — KPI 타일 스파크라인 소재(목업 상단 타일은
      // 전부 미니 시각화를 달고 있다). 질의 2건 추가, 폴링 주기는 그대로.
      const [rpsRes, p95Res, tokRes, errRes] = await Promise.all([
        api.rangeQuery(METRIC_DEMO_FLEET_RPS, startSec, endSec, step),
        api.rangeQuery(METRIC_DEMO_FLEET_P95, startSec, endSec, step),
        api.rangeQuery(METRIC_DEMO_FLEET_TOKENS, startSec, endSec, step),
        api.rangeQuery(METRIC_DEMO_FLEET_ERR_PCT, startSec, endSec, step),
      ]);
      return mergeMatrices([
        { key: 'rps', matrix: rpsRes.data },
        { key: 'p95', matrix: p95Res.data },
        { key: 'tokens', matrix: tokRes.data },
        { key: 'err', matrix: errRes.data },
      ]);
    },
    RANGE_POLL_MS,
    [demoOn],
  ...
  // 타일 스파크라인 — 추이 차트가 이미 받아온 계열을 그대로 재사용한다.
  // 원시 계열은 스텝당 분산이 커서 그대로 그리면 노이즈 덩어리가 되고
  // "부하와 지연이 같이 움직인다"는 이 화면의 논지가 파묻힌다 — 이동평균으로
  // 고주파만 걷어낸다(값 범위·축은 그대로).
  const sparkOf = (key: string): number[] =>
    movingAvg(fleetRows.map((r) => Number(r[key])).filter((v) => Number.isFinite(v)), 9);
  // 추이 차트도 같은 이유로 다듬는다 — 원시 계열끼리는 두 선이 서로를 가려
  // 동조(요청↑→지연↑)가 안 보였다.
  const smoothRps = movingAvg(fleetRows.map((r) => Number(r.rps)), 9);
  const smoothP95 = movingAvg(fleetRows.map((r) => Number(r.p95)), 9);
  const fleetSmoothed = fleetRows.map((r, i) => ({
    ...r,
    rps: smoothRps[i],
    p95: smoothP95[i],
  }));
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        <KpiTile
          ...
          hint="LLM·RAG 계열 합계 · 최근 3시간"
          icon={<Coins className="size-4" />}
          spark={sparkOf('tokens')}
        />
        <KpiTile
          label="에러율"
        ...
        <CardContent>
          {fleetTrend.data === null && fleetTrend.error === null ? (
            ...
            <ResponsiveContainer width="100%" height={260}>
              <LineChart data={fleetSmoothed} margin={CHART_MARGIN}>
