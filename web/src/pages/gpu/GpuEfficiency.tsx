...
import { Boxes, CalendarClock, Cpu, Gauge, PiggyBank, TrendingDown } from 'lucide-react';
...
// 목업(demo4)의 표 밀도 = 한 화면에 6~8행 + 총건수 캡션. 50행을 쏟으면 페이지가
// 스크롤 늪이 되어 같은 제품으로 보이지 않는다.
const LOW_UTIL_ROWS = 8;
const IDLE_ROWS = 8;
...
// 미니 막대용 정규화 — KpiTile 의 Bars 는 "최댓값 기준 상대 높이"라, 값들이
// 서로 비슷하면(갭 18~22pp, 사용률 0.0~0.3%) 전부 만높이로 그려져 색 덩어리가
// 된다. 최소~최대를 1~10 으로 펴서 목업 타일처럼 기복이 보이게 한다.
...
export default function GpuEfficiency() {
  ...
  // 장기 미사용 — idleDays 내림차순 (demo 전용 데이터). idle[] 에는 1~29일
  // 최근 유휴도 섞여 오므로(유휴장비 관리 화면의 "전체 유휴" 축) 이 표의 제목
  // 대로 30일 이상만 남긴다.
  const idleRows = useMemo(() => {
    const idle = state?.idle ?? [];
    return idle.filter((g) => g.idleDays >= 30).sort((a, b) => b.idleDays - a.idleDays);
  }, [state?.idle]);
  ...
  // 갭(할당−실사용) 최근 추이 — 타일 하단 미니 막대. 목업 타일은 전부 미니
  // 시각화를 달고 있어, 몇 개만 비면 카드 아랫단이 들쭉날쭉해진다.
  const gapBars = spread(
    gapRows
      .filter((r) => r.alloc !== null && r.util !== null)
      .slice(-24)
      .map((r) => Math.max((r.alloc ?? 0) - (r.util ?? 0), 0)),
  );
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {/* 1. KPI 타일 행 — 목업(demo1/demo4) 상단 6타일 어휘로 통일 (demo 전용) */}
      {demoOn ? (
        <>
          <div
          ...
          >
            <KpiTile
              ...
              tone="info"
              value={alloc === null ? null : formatPct(alloc)}
              hint="풀에 배정된 비율"
              progress={alloc ?? undefined}
            />
            <KpiTile
              label="실사용률 (평균)"
              icon={<Gauge className="size-4" />}
              value={utilAvg === null ? null : formatPct(utilAvg)}
              hint="실제 연산 사용률"
              progress={utilAvg ?? undefined}
            />
            <KpiTile
              label="할당 − 실사용 갭"
              icon={<TrendingDown className="size-4" />}
              tone="danger"
              value={gapPp === null ? null : `${gapPp.toFixed(1)}pp`}
              hint="잠들어 있는 용량"
              bars={gapBars}
            />
            <KpiTile
              ...
              value={
                gapPp === null || !state
                  ? null
                  : `${Math.round((gapPp / 100) * state.fleet.gpuTotal).toLocaleString('ko-KR')}장`
              }
              hint={state ? `전체 ${state.fleet.gpuTotal.toLocaleString('ko-KR')}장 기준` : undefined}
              bars={gapBars}
            />
            <KpiTile
              label="저활용 GPU"
              icon={<Boxes className="size-4" />}
              value={lowUtil === null ? null : `${lowUtil.total.toLocaleString('ko-KR')}장`}
              hint={`< ${LOW_UTIL_THRESHOLD}% 사용률`}
              bars={lowUtil ? spread(lowUtil.rows.map((c) => c.util)) : undefined}
            />
            <KpiTile
              label="30일+ 미사용"
              icon={<CalendarClock className="size-4" />}
              tone="warn"
              value={state === null ? null : `${idleRows.length.toLocaleString('ko-KR')}장`}
              hint="사유 등록 대상"
              bars={spread(idleRows.slice(0, 12).map((g) => g.idleDays))}
            />
          </div>
          <p className="text-muted-foreground text-sm">
        ...
        <TabsContent value="low-util">
          <Card>
            ...
            <CardContent>
              ...
              {lowUtil !== null && lowUtil.total > 0 ? (
                <p className="pt-3 text-muted-foreground text-xs">
                  1-{lowUtil.rows.length} / 총 {lowUtil.total.toLocaleString('ko-KR')}장 (사용률
                  오름차순)
                </p>
              ) : null}
            </CardContent>
          ...
          <TabsContent value="idle">
            <Card>
              ...
              <CardContent>
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {idleRows.length === 0 ? (
                        ...
                        idleRows.slice(0, IDLE_ROWS).map((g) => {
                ...
                {idleRows.length > 0 ? (
                  <p className="pt-3 text-muted-foreground text-xs">
                    1-{Math.min(IDLE_ROWS, idleRows.length)} / 총{' '}
                    {idleRows.length.toLocaleString('ko-KR')}장 (미사용 일수 내림차순)
                  </p>
                ) : null}
              </CardContent>
            </Card>
