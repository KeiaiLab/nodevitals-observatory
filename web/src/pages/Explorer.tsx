...
// 한 셀렉터가 수백~수천 시리즈에 매치될 수 있다(플릿 전 노드의 node_load1 =
// 879 시리즈). 전부 그리면 선이 뭉개지는 것보다 범례가 먼저 터진다 — Recharts
// Legend 가 시리즈명 879개를 화면 위로 흘려보내 상단 네비·카드 제목까지 덮었다.
// 상위 N 개만 그리고 나머지는 개수로만 알린다.
const MAX_CHART_SERIES = 8;

/** 매치 목록 표에 그리는 행 수 — 표는 셀렉터가 무엇에 걸렸는지 보여주는 표본이다. */
const MAX_SERIES_ROWS = 20;
...
function buildChartData(matrix: MatrixData | null): {
  ...
  if (!matrix || matrix.result.length === 0) {
    return { rows: [], seriesMeta: [], totalSeries: 0 };
  }

  const totalSeries = matrix.result.length;
  const drawn = matrix.result.slice(0, MAX_CHART_SERIES);

  const seriesMeta: SeriesMeta[] = drawn.map((s, index) => ({
    key: `s${index}`,
  ...
  return { rows, seriesMeta, totalSeries };
}
...
export default function Explorer() {
  ...
  const { rows, seriesMeta, totalSeries } = useMemo(() => buildChartData(matrix), [matrix]);

  return (
    <div className="flex flex-col gap-4">
      {/* 제목은 셸이 메뉴 라벨로 이미 그린다(중복 h1 금지 — Overview 와 동일). */}
      <p className="text-muted-foreground text-sm">
        수집 중인 메트릭을 직접 골라 원하는 기간의 시계열을 살펴본다.
      </p>

      <Card>
        <CardHeader>
          <CardTitle>메트릭 선택</CardTitle>
          <CardDescription>
            지표와 기간을 고르면 아래에 시계열이 그려진다. 셀렉터는{' '}
        ...
        <CardContent className="flex flex-col gap-4">
          <div className="flex flex-wrap items-center gap-2">
            {/* 열 때 검색어를 비운다. 현재 셀렉터를 넣어 두면 cmdk 가 그 문자열로
                목록을 걸러, 프리셋 9종 중 "이미 고른 그 항목" 1개만 남은 채로
                열렸다(실측: node_load1 상태에서 열면 표시 2행 = node_load1 +
                자유 입력). 지표를 바꾸려고 여는 물건이 바꿀 대상을 감춘 셈이다. */}
            <Popover
              open={comboOpen}
              onOpenChange={(open) => {
                setComboOpen(open);
                if (open) setComboInput('');
              }}
            >
          ...
          <div className="flex flex-col gap-2">
            <p className="text-xs text-muted-foreground">필터로 쓸 수 있는 라벨</p>
            {labelsError ? (
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {totalSeries === 0
              ? '매치된 시리즈가 없다.'
              : totalSeries > seriesMeta.length
                ? `${totalSeries.toLocaleString('ko-KR')}개 시리즈 중 상위 ${seriesMeta.length}개 · ${RANGE_LABELS[rangeKey]}`
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            {seriesList.length > MAX_SERIES_ROWS
              ? `총 ${seriesList.length.toLocaleString('ko-KR')}건 중 상위 ${MAX_SERIES_ROWS}건`
              : `${seriesList.length}건`}
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
            ...
            <TableBody>
              {seriesList.length === 0 ? (
                ...
                seriesList.slice(0, MAX_SERIES_ROWS).map((lset) => {
