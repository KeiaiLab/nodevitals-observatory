...
export interface AlertTimelineProps {
  ...
  /** true 면 critical·major 항목에 심각도 배지를 붙인다(GPU Assistant 이벤트 카드).
   *  warning·info 는 점 색으로 충분해 배지를 달지 않는다 — 전 행에 배지가 붙으면
   *  "무엇이 심각한가"라는 신호가 죽는다. */
  severityBadge?: boolean;
}
...
export default function AlertTimeline({ alerts, onAck, severityBadge }: AlertTimelineProps) {
        ...
        >
          ...
          <div className="flex flex-col gap-0.5">
            <div className="flex flex-wrap items-center gap-2">
              ...
              {severityBadge && (alert.severity === 'critical' || alert.severity === 'major') ? (
                <Badge
                  variant="outline"
                  ...
                  style={{
                    ...
                    backgroundColor: `color-mix(in srgb, ${SEVERITY_COLOR[alert.severity]} 12%, transparent)`,
                  }}
                >
